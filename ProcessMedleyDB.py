...
for metafile in metadatalist:
    ...
    for s in data["stems"]:
        ...
        print("stem: %s %s %s" % (fname,stem["component"],stem["instrument"]))
        if (stem["instrument"].find("male")>=0) or (stem["instrument"].find("singer")>0): 
    ...
    if (len(stem_voc)==0)or(len(stem_inst)==0):
        ...
        continue
    audio_vocal = sum([load(os.path.join(stems_path,f),sr=None,mono=True)[0] for f in stem_voc])
    audio_inst = sum([load(os.path.join(stems_path,f),sr=None,mono=True)[0] for f in stem_inst])
    audio_mix,_ = load(os.path.join(PATH_MENDLEY,songname,mixfilename),sr=None,mono=True)
