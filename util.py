...
def SaveSpectrogram(y_mix,y_vocal,y_inst,fname,original_sr=44100):
    ...
    S_mix = np.abs(stft(y_mix,n_fft=C.FFT_SIZE,hop_length=C.H)).astype(np.float32)
    S_vocal = np.abs(stft(y_vocal,n_fft=C.FFT_SIZE,hop_length=C.H)).astype(np.float32)
...
def ComputeMask(input_mag,unet_model="unet.model",hard=True):
    ...
    if hard:
        hard_mask = np.zeros((mask.shape[0]+1,mask.shape[1]),dtype="float32")
